...
## Bug修复 - 振假名显示问题 (进行中)
- [ ] 分析问题：注音中包含标点符号（顿号、问号等）
- [ ] 分析问题：注音位置错误（没有正确对应到各个词汇）
- [ ] 查看后端API返回的原始数据格式
- [x] 修复buildRubyText函数的注音分配逻辑 - 采用按标点分段的新算法
- [x] 确保标点符号不出现在注音中 - 后端API保留标点，前端按标点分词对应
...
## Bug修复 - 申します的注音显示为汉字 (已完成)
- [x] 检查数据库中「はじめまして。田中と申します。」的reading数据 - 发现LLM生成时漏转了「申」
- [x] 分析问题原因 - 脚本的prompt不够明确，没有强调所有汉字都必须转换
- [x] 修复prompt，添加「田中と申します」的示例
- [x] 重新运行脚本更新数据库
- [x] 测试修复效果 - 所有汉字都正确显示假名注音

## 移动端适配优化 (已完成)
- [x] 修复AI助手页面标题和按钮布局 - 使用flex-col和truncate，移动端按钮只显示图标
- [x] 修复词汇库页面标题和logo显示 - 调整字体大小，添加whitespace-nowrap
...
## 账号密码登录系统替换OAuth
- [x] 分析当前OAuth认证系统的实现
- [x] 修改user表schema，添加email和password字段
- [x] 创建密码加密工具函数（使用bcrypt）
- [x] 开发注册API（auth.register）
- [x] 开发登录API（auth.login）
- [x] 修改登出API（auth.logout）
- [x] 创建前端登录页面（Login.tsx）
- [x] 创建前端注册页面（Register.tsx）
- [x] 更新Layout组件移除OAuth登录链接
